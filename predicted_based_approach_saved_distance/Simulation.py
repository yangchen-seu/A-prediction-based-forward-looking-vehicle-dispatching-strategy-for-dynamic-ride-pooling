...
class Simulation():
    ...
    def batch_matching(self, takers, vehicles, seekers):
        ...
        for i in range(len(takers)):
            ...
            if res[i] >= len(seekers):
                ...
                # seekers[res[i]].matchingWeight = weights[i,res[i]]
        ...
        for i in range(len(vehicles)):
            ...
            if res[i + len(takers)] >= len(seekers):
                ...
                # seekers[res[i + len(takers)]].soloWeight = weights[i][res[i + len(takers)]]
    ...
    def calTakersWeights(self, taker, seeker,  optimazition_target, matching_condition):
        ...
        if optimazition_target == 'platform_income':
            ...
            if matching_condition and (pick_up_distance > self.pickup_distance_threshold or
                ...
                if seeker.matchingWeight == 0:
                    seeker.matchingWeight = reward
                # reward = distance_saving * (distance_saving/ (distance_saving + pick_up_distance)) 
                # reward =( distance_saving + 3000) * seeker.delay
                # print('distance_saving',distance_saving)
                # print('balanced taker reward',reward)
                return reward
    ...
    def calVehiclesWeights(self, vehicle, seeker,  optimazition_target, matching_condition):
        ...
        if optimazition_target == 'platform_income':
            ...
            if matching_condition and (pick_up_distance > self.cfg.pickup_distance_threshold ):
                ...
                #     seeker.esdt += 100
                #     return seeker.esdt
                    # reward = (seeker.esdt - pick_up_distance + 3000 ) * seeker.delay
                    # print('origin vacant vehicle reward',reward)
                    # reward = (seeker.esdt - pick_up_distance  +3000) * seeker.delay
                reward = seeker.esdt * (seeker.esdt / (seeker.esdt + pick_up_distance )) * seeker.delay
                if seeker.soloWeight == 0:
                    seeker.soloWeight = reward
                    # reward = seeker.esdt * (seeker.esdt / (seeker.esdt + pick_up_distance )) 
                    # reward = seeker.esdt 
                    # print('seeker.esdt',seeker.esdt)
                    # print('balanced vacant vehicle reward',reward)
                return reward
    ...
    def calSeekerWaitingWeights(self, seeker,  optimazition_target):
        if optimazition_target == 'platform_income':
            ...
            # reward = (seeker.esds + 3000)  * (1 - (1- self.cfg.rw) ** (max(0,self.cfg.delay_time_threshold / 10 - seeker.k ))) 
            # print('seeker.k',seeker.k)
            # reward = seeker.esds * (1 - (1- self.cfg.rw) ** (max(0,40 - seeker.k )))  * (self.vehicle_num - self.activate_drivers) / self.vehicle_num
            # print('waiting matching delay',gamma * seeker.k * 60)
            # reward = seeker.esds - gamma * (self.time - seeker.begin_time_stamp) /10
            # print('seeker.esds',seeker.esds)
            # print('waitiing reward',reward)
