...
class Simulation():
    ...
    def generate_RTV_graph(self, vehicles, takers, e_r_r):
        ...
        for v in vehicles:
            ...
            for r in v.e_r_v:
                ...
                # print('trip1生成')
                r.trips.append(trip)
                weight = self.calVehiclesWeights(v, r)
                # print('weight',weight)
        ...
        for v in takers:
            ...
            for r in v.e_r_v:
                ...
                if res:
                    ...
                    trip = Trip.Trip(r, False, v)
                    r.trips.append(trip)
                    weight = self.calTakersWeights(v, r)
                    trip.set_Value(weight)
                    # print('weight',weight)
                    trips.append(trip)
    ...
    def optimal_assignment(self, takers, vehicles, seekers):
        ...
        for vehicle in vehicles:
            if vehicle not in matched_1_passenger_vehicle and vehicle not in matched_2_passenger_vehicle:
                ...
                # 判断未被响应的时间
                if self.time - vehicle.activate_time > 10:
                    # print('超过2m')
                    # 两分钟未被响应，随机调度
                ...
                # 更新空车司机位置
                    ...
                    if vehicle.path:
                        vehicle.activate_time = self.time + self.cfg.unit_driving_time * vehicle.path_length[0]
                        # 没到目的地
                        if self.time > vehicle.activate_time:
                            vehicle.activate_time = self.time + self.cfg.unit_driving_time * vehicle.path_length[0]
                ...
                vehicle.origin_location = vehicle.order_list[0].O_location
                ...
                pickup_p1_distance = self.get_path(
                    vehicle.location, vehicle.order_list[0].O_location)
                pickup_p2_distance = self.get_path(
                    vehicle.location, vehicle.order_list[1].O_location)
                ...
                if pickup_p1_distance < pickup_p2_distance:
                    pickup_p1_time = self.cfg.unit_driving_time * pickup_p1_distance
                    ...
                    vehicle.drive_distance += pickup_p1_distance
                    vehicle.drive_distance += pickup_p2_time / self.cfg.unit_driving_time
                    self.total_travel_distance += pickup_p1_distance
                    self.total_travel_distance += pickup_p2_time / self.cfg.unit_driving_time

                else:
                    pickup_p2_time = self.cfg.unit_driving_time * pickup_p2_distance
                    self.pickup_time.append(pickup_p2_time)
                    ...
                    vehicle.drive_distance += pickup_p2_distance
                    vehicle.drive_distance += pickup_p1_time / self.cfg.unit_driving_time
                    self.total_travel_distance += pickup_p2_distance
                    self.total_travel_distance += pickup_p1_time / self.cfg.unit_driving_time

                self.saved_travel_distance += vehicle.order_list[0].shortest_distance
                self.saved_travel_distance += vehicle.order_list[1].shortest_distance

                # 决定派送顺序，是否fifo
                fifo, distance = self.is_fifo(
                    vehicle.order_list[0], vehicle.order_list[1])
                pickup_distance = pickup_p1_distance if pickup_p1_distance < pickup_p2_distance else pickup_p2_distance
                pickup_time = self.cfg.unit_driving_time * pickup_distance
                if fifo:
                    # 先上先下
                    self.total_travel_distance += self.get_path(
                    ...
                    self.total_travel_distance += self.get_path(
                        vehicle.order_list[0].D_location, vehicle.order_list[1].D_location)

                    p0_invehicle = pickup_distance + distance[0]
                    p0_expected_distance = vehicle.order_list[0].shortest_distance
                    # 绕行
                    vehicle.order_list[0].set_detour(
                        p0_invehicle - p0_expected_distance)
                    # print('detour',p0_invehicle - p0_expected_distance)
                    p1_invehicle = sum(distance)
                    p1_expected_distance = vehicle.order_list[1].shortest_distance
                    vehicle.order_list[1].set_detour(
                        p1_invehicle - p1_expected_distance)
                    # print('detour',p1_invehicle - p1_expected_distance)
                    # travel time
                    vehicle.order_list[0].set_traveltime(
                        self.cfg.unit_driving_time * p0_invehicle)
                    vehicle.order_list[1].set_traveltime(
                        self.cfg.unit_driving_time * p1_invehicle)
                    destination = vehicle.order_list[1].D_location
                else:
                    # 先上后下
                    self.total_travel_distance += self.get_path(
                        vehicle.order_list[1].O_location, vehicle.order_list[1].D_location)
                    self.total_travel_distance += self.get_path(
                        vehicle.order_list[1].D_location, vehicle.order_list[0].D_location)
                    ...
                    p1_expected_distance = vehicle.order_list[1].shortest_distance
                    vehicle.order_list[1].set_detour(
                        p1_invehicle - vehicle.order_list[1].shortest_distance)
                    # print('detour',p1_invehicle - p1_expected_distance)
                    # travel time
                    vehicle.order_list[0].set_traveltime(
                        self.cfg.unit_driving_time * p0_invehicle)
                    vehicle.order_list[1].set_traveltime(
                        self.cfg.unit_driving_time * p1_invehicle)
                    destination = vehicle.order_list[0].D_location

                # 乘客的行驶距离
                vehicle.order_list[0].ride_distance = p0_invehicle
                vehicle.order_list[1].ride_distance = p1_invehicle
                vehicle.order_list[0].shared_distance = distance[0]
                vehicle.order_list[1].shared_distance = distance[0]
                ...
                self.platform_income.append(
                    ...
                    self.cfg.unit_distance_cost/1000 * vehicle.drive_distance
                )
                # print('order.value{},driver_distance{},income:{}'.format(self.cfg.discount_factor * (taker.order_list[0].value + taker.order_list[1].value),taker.drive_distance,\
                #     self.cfg.discount_factor * (taker.order_list[0].value + taker.order_list[1].value) - \
                #     self.cfg.unit_distance_cost/1000 * taker.drive_distance))
    ...
    def calTakersWeights(self, taker, seeker):
        ...
        if (pick_up_distance > self.pickup_distance_threshold or
    ...
    def calVehiclesWeights(self, vehicle, seeker):
