...
class Simulation():
                ...
                for key in match.keys():
                    if match[key] == weight: # key 接到乘客
                        if key.target == 1: # taker
                            ...
                            if key.location != key.order_list[0].O_location:     
                            ...
                            self.carpool_order.append(seeker)
                            # 记录目的地
                            pickup_distance, key.path, key.path_length = self.network.get_path(
        ...
        for taker in takers:
            ...
            if taker.reposition_target == 1:
                ...
                if self.time - taker.order_list[0].begin_time_stamp - \
                    ...
                    taker.order_list[0].ride_distance = travel_distance
    ...
    def calVehiclesWeights(self, vehicle, seeker,  optimazition_target, matching_condition):
        ...
        if matching_condition and (pick_up_distance > self.cfg.pickup_distance_threshold or seeker.esdt < 0):
            ...
            reward = (0 - pick_up_distance) * seeker.delay
            return reward
