...
for i in range(len(ODs)):
    print(ODs[i], orders[i], predict_results[i])
    order = pd.read_csv('orders/' + orders[i])
    OD = pd.read_csv('ODs/' + ODs[i])
    predict_result = pd.read_csv('predict_results/' + predict_results[i])
    predict_result = pd.merge(
        OD, predict_result, left_on='OD_id', right_on='OD_id')
    predict_result['destination_id'] = predict_result['destination_id'].astype(
        int)
    ...
    res = res[res['dwv_order_make_haikou_1.arrive_time']
              != '0000-00-00 00:00:00']
    res['dwv_order_make_haikou_1.departure_time'] = res['dwv_order_make_haikou_1.arrive_time']
    res[['dwv_order_make_haikou_1.order_id', 'dwv_order_make_haikou_1.arrive_time','dwv_order_make_haikou_1.departure_time',
