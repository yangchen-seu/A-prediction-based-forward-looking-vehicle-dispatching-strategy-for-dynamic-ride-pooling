...
def target(row):
       if row.saved_distance_for_seeker < row.saved_distance_for_taker:
              return 1
       else:
              return 0
predict_result['target'] = predict_result.apply(target, axis = 1)
predict_result.head()
# In[]
print(len(predict_result[predict_result['target'] == 1]))
print(len(predict_result[predict_result['target'] == 0]))
