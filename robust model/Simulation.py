...
class Simulation():
    ...
    def step(self,):
        ...
        for index, row in current_time_orders.iterrows():
            ...
            seeker.waitingtime_threshold = max(
                30, np.random.normal(self.cfg.delay_time_threshold, 10))
    ...
    def process(self, time_, ):
        ...
        if self.time >= time.mktime(time.strptime(self.cfg.date + self.cfg.simulation_end_time, "%Y-%m-%d %H:%M:%S")):
            ...
            self.res['avg_distance_saving'] = np.mean(
            ...
            if self.cfg.progress_target:
                print('匹配用时{},time{},vehicles{},takers{},seekers{}'.format(
                    end - start, self.time_slot, len(vehicles), len(takers), len(seekers)))
            # for vehicle in self.vehicle_list:
            #     print('vehicle.activate_time',vehicle.activate_time)
    ...
    def batch_matching(self, takers, vehicles, seekers):
        ...
        for column in range(P):
            ...
            for row in range(V_1):
                if takers[row].location in nodes:
                    matrix[row, column], detour[row, column], pickup[row, column] = self.calTakersWeights(takers[row], seekers[column],
                                                                                                          optimazition_target=self.optimazition_target,
                                                                                                          matching_condition=self.matching_condition)
                    d[row, column] = matrix[row, column] * 0.3
                    if detour[row, column] <= self.cfg.detour_distance_threshold and pickup[row, column] <= self.cfg.pickup_distance_threshold:
                        Gamma[row, column] = 1 if np.random.random(
                        ) <= self.cfg.gamma else 0

                else:
                    continue

            for row in range(V_0):
                if vehicles[row].location in nodes:
                    matrix[row + V_1, column], detour[row + V_1, column], pickup[row + V_1, column] = self.calVehiclesWeights(vehicles[row], seekers[column],
                                                                                                                              optimazition_target=self.optimazition_target,
                                                                                                                              matching_condition=self.matching_condition)
                    d[row + V_1, column] = matrix[row, column] * 0.3
                    if detour[row, column] <= self.cfg.detour_distance_threshold and pickup[row, column] <= self.cfg.pickup_distance_threshold:
                        Gamma[row, column] = 1 if np.random.random(
                        ) <= self.cfg.gamma else 0

            for row in range(V_wait):
                matrix[row + V_1 + V_0, column], detour[row + V_1 + V_0, column], pickup[row + V_1 + V_0, column] = self.calSeekerWaitingWeights(seekers[column],
                                                                                                                                                 optimazition_target=self.optimazition_target)
                d[row + V_1 + V_0, column] = matrix[row, column] * 0.3
                if detour[row, column] <= self.cfg.detour_distance_threshold and pickup[row, column] <= self.cfg.pickup_distance_threshold:
                    Gamma[row, column] = 1 if np.random.random(
                    ) <= self.cfg.gamma else 0
        ...
        z = model.addVars(V, P, lb=0)
        q = model.addVars(V, P, lb=0)

        # Define the objective function
        # objective = gp.quicksum(-matrix[v][p] * x[v, p] + z * Gamma + q[v, p] for p in range(P) for v in range(V))
        objective = gp.quicksum(-matrix[v][p] * x[v, p] + z[v, p] *
                                Gamma[v][p] + q[v, p] for p in range(P) for v in range(V))
        model.setObjective(objective, GRB.MINIMIZE)

        # Define the constraints
        for p in range(P):
            model.addConstr(gp.quicksum(x[v, p] for v in range(V)) <= 1)

        for v in range(V):
            model.addConstr(gp.quicksum(x[v, p] for p in range(P)) <= 1)

        for p in range(P):
            for v in range(V):
                model.addConstr(z[v, p] + q[v, p] >= d[v][p] * y[v, p])
                model.addConstr(-x[v, p] <= y[v, p])
                model.addConstr(x[v, p] <= y[v, p])
                model.addConstr(detour[v, p] * x[v, p] <=
                                self.cfg.detour_distance_threshold)
                model.addConstr(pickup[v, p] * x[v, p] <=
                                self.cfg.pickup_distance_threshold)
        ...
        optimal_values = {(v, p): x[v, p].x for v in range(V)
                          for p in range(P)}
        end = time.time()
        ...
        #             print(f"y[{v}][{p}] = {y[v, p].x}")
        #             print(f"d[{v}][{p}] = {d[v, p]}")
        #             print(f"q[{v}][{p}] = {q[v, p].x}")
        #             print(f"z[{v}][{p}] = {z[v, p].x}")
        #             print(f"gamma[{v}][{p}] = {Gamma[v, p]}")
        ...
        for v in range(len(vehicles)):
            if vehicles[v].reposition_target == 1:
        ...
        for taker in takers:
            ...
            if taker.reposition_target == 1:
                ...
                if self.time - taker.order_list[0].begin_time_stamp - \
                    ...
                    self.platform_income.append(
                        ...
                        self.cfg.unit_distance_cost/1000 *
                        (taker.order_list[0].shortest_distance +
                         taker.p0_pickup_distance)
                    )
                ...
                # 没匹配到其他乘客，但接到p1了，开始在路上派送
                    ...
                    if taker.path:
                        ...
                        # self.saved_travel_distance += taker.order_list[0].ride_distance
                        # 计算平台收益
                        self.platform_income.append(self.cfg.discount_factor * taker.order_list[0].value - self.cfg.unit_distance_cost/1000 * (
                            taker.order_list[0].shortest_distance + taker.p0_pickup_distance))
                        # print('order.value{},driver_distance{},income{}:'.format(taker.order_list[0].value,taker.order_list[0].shortest_distance + taker.p0_pickup_distance,self.platform_income[-1]))
                        # 完成订单
                        self.his_order.append(taker.order_list[0])
                        taker.order_list = []
            ...
            # 当前匹配有新乘客
                ...
                self.saved_travel_distance += taker.order_list[1].shortest_distance + \
                    taker.order_list[0].shortest_distance - \
                    (p0_invehicle + p1_invehicle - distance[0])
                ...
                self.platform_income.append(
                    ...
                    (p0_invehicle + p1_invehicle -
                     distance[0] + taker.p0_pickup_distance)
                )
    ...
    def calTakersWeights(self, taker, seeker,  optimazition_target, matching_condition):
        ...
        distance_saving = seeker.shortest_distance + \
            taker.order_list[0].shortest_distance - \
            (p0_invehicle + p1_invehicle - shared_distance)

        reward = distance_saving * \
            (distance_saving / (distance_saving +
                                pick_up_distance + 1)) * seeker.delay

        return reward, max(p0_detour, p1_detour), pick_up_distance
    ...
    def calVehiclesWeights(self, vehicle, seeker,  optimazition_target, matching_condition):
        ...
        reward = seeker.esdt * \
            (seeker.esdt / (seeker.esdt + pick_up_distance)) * seeker.delay

        return reward, 0, pick_up_distance

    # 计算乘客选择等待的权重

    def calSeekerWaitingWeights(self, seeker,  optimazition_target):

        reward = seeker.esds * (1 - (1 - self.cfg.rw) ** (max(0, self.cfg.delay_time_threshold /
                                                              10 - seeker.k))) / (seeker.k + 1)  # np.exp(seeker.k) (seeker.k + 1)
        return reward, 0, 0
