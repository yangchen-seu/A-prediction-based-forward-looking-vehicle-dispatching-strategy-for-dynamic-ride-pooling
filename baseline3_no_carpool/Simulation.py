...
class Simulation():
    ...
    def batch_matching(self, vehicles, seekers):
        ...
        for vehicle in vehicles:
            if vehicle.reposition_target == 1:
                ...
                # 匹配到乘客了，接到乘客了，开始派送
                pickup_distance = self.get_path(vehicle.location, vehicle.order_list[0].O_location)
                ...
                vehicle.activate_time = self.time + (pickup_time + self.cfg.unit_driving_time *vehicle.order_list[0].shortest_distance)

                vehicle.location = vehicle.order_list[0].D_location
                self.platform_income.append(vehicle.order_list[0].value - self.cfg.unit_distance_cost/1000 * (pickup_distance + vehicle.order_list[0].shortest_distance))
    ...
    def calVehiclesWeights(self, vehicle, seeker,  optimazition_target, matching_condition):
        if optimazition_target == 'platform_income':
            ...
            pick_up_distance = self.get_path( vehicle.location, seeker.O_location)
            if matching_condition and (pick_up_distance > self.cfg.pickup_distance_threshold):
                ...
                reward = (dispatch_distance - pick_up_distance) + 3000
                # print('reward',reward)
                return reward            
